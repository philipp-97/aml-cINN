...
class MNISTcINN_hx(nn.Module):
    ...
    def build_inn(self):
        ...
        def fc_subnet(ch_in, ch_out):
            ...
            return nn.Sequential(
                hxnn.Linear(in_features=ch_in,
                            out_features=self.c.internal_width,
                            bias=True,
                            num_sends=2,            # scales the hardware-gain, will be
                ...
                hxnn.ConvertingReLU(shift=1,  # shifts the output by 1 bit,
                                              # i.e. divides it by 2.
                                    mock=self.c.mock),
                hxnn.Linear(in_features=self.c.internal_width,
                            out_features=ch_out,
                            bias=True,
                            num_sends=3,
    ...
    def reverse_sample(self, z, l, jac=True):
        return self.cinn(z, c=one_hot(l, scale=31.), rev=True, jac=jac)
